...
import {View, Text, TextInput, StatusBar,FlatList,Pressable,ImageBackground,TouchableOpacity,ScrollView,ActivityIndicator,
...
import {sizes, colors, fontFamily,appImages, ToastMessage} from '../../../themes';
...
const CreateService = props => {
  ...
  const [price, setprice] = useState(param?.status!=null?param?.status:false);
  console.log("raaaaaaaaaaaat",price);
...
useEffect(() => {
 getService()
  ...
  const getService = () => {
    ...
    try {
        ...
        showServiceData().then(response => {
            console.log('showservices data22 =====> ', response.data[0].subservices[0].category);
            setLoader(false)
            if (response?.success) {
  ...
  const AddService = () => {
    ...
    formdata.append("rates[]",JSON.stringify(param?.editdata?.category?.length>0?param?.editdata?.category:category))
    ...
    formdata.append('base_price', param?.base_cast!=null?param?.base_cast:"20");
    formdata.append('base_time', param?.base_time!=null?param?.base_time:'20');
...
// name:ProfileImageArry.name})
    ...
    return (
      <View
        ...
        <Pressable
          ...
          <ImageBackground
            source={{uri:item?.uri}}
            style={{resizeMode: 'cover',height:height(14),width:width(28),marginHorizontal:width(1),marginTop:height(0.5)}}
            imageStyle={{borderRadius: 15}}>
              
            <TouchableOpacity 
            onPress={()=>deleteImage(item)}
            style={{height:height(2),width:width(4),borderRadius:100,backgroundColor:'#fff',alignSelf:'flex-end',margin:totalSize(1),
    ...
    return (
      <View
        ...
        <Pressable
          ...
          <Pressable
            // source={item?.images}
            style={{height:height(14),width:width(28),backgroundColor:"#00000029",borderRadius:15,marginTop:height(0.5),marginRight:width(1.5),alignItems:'center',
  ...
  return (
    <MainWrapper>
      ...
      :<ImageBackground source={ProfileImage} style={{height: height(20),
        alignItems: 'center',
        justifyContent: 'center',
        borderWidth: 1,
        borderColor: colors.lightgrey,
        ...
        imageStyle={{borderRadius:100 }} blurRadius={chargesModalStatus?30:0}>
          {!chargesModalStatus?
          <>
      ...
      <Spacer height={sizes.baseMargin} />
        ...
        <VDropDown
            ...
            const a =[{
              ...
              "sub_category_id": subservice[index]?.category[0]?.sub_categories[0]?.id,
              "price": subservice[index]?.category[0]?.sub_categories[0]?.price,
              "time": subservice[index]?.category[0]?.sub_categories[0]?.time,
            }]
            setobject(a)
            }}
...
<Spacer height={sizes.smallMargin} />
          ...
          style={{right: totalSize(3.2), top: totalSize(5.2)}}>
            <Entypo
             onPress= {()=>
              ...
              subserviceid!=""&&
              subserviceid!=null
              ?props.navigation.navigate(routes.pricenewList,{data:{serviceid:serviceid,subserviceid:subserviceid,category:category,
            servicename:servicename,subservicename:subservicename}})
        ...
        ListHeaderComponent={renderHeader}
          ...
          data={ProfileImageArry&&ProfileImageArry!=null?ProfileImageArry:ProfileImageArry}
          horizontal={true}
          ...
          <ModalColored
           ...
           source={{uri:ProfileImage.uri}}
            ...
            <View
              ...
              <SmallText style={{fontSize:totalSize(1.2),width:width(28),textAlign:'center'}}>{subservicename}</SmallText>
            </View>
          </ImageBackground>
           </View>
                <ButtonColored
